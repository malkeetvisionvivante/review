@if(count($peer)) 
                ...
                @if ($type == 'user_id')
                    ...
                    @if ($datas && $datas->isFlagged())
                    <!-- <small class="float-right pt-1 mr-3">
                      <i class="fas fa-flag text-muted" style="font-size:15px;color:red !important;"></i> 
                    </small> -->
                    @endif
                   ...
                   <table class="table custom-table table-border-0"> 
                    ...
                    @if($datas->comment != null && $datas->comment != '') 
                            ...
                            <tr id="_comment_{{$datas->id}}_">
                              ...
                              <td>
                                <a id="_cmnt{{$datas->id}}" title="{{$datas->hidden_comment == 1 ? 'Show Comment' : 'Hide Comment' }}" href="javascript:void(0)" class="{{$datas->hidden_review==1?'_reviewHidden':''}} hide-comment" data-id="{{$datas->id}}">
