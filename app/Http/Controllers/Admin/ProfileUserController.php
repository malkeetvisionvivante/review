...
class ProfileUserController extends Controller {
    ...
    public function users_list(Request $request){
      ...
      if($request->type == 'name' && isset($request->name) && !empty($request->name)){
        ...
        if(count($bindArr)==1) {
          $data = User::where('name',"like", $bindArr[0].'%')->orWhere('last_name',"like", $bindArr[0].'%')->whereIn('type',['Imported','CreatedByUser','ProfileCreatedByAdmin']);
        } else if(count($bindArr)>1) {
          $data = User::where('name',"like", $bindArr[0].'%')->Where('last_name',"like", $bindArr[1].'%')->whereIn('type',['Imported','CreatedByUser','ProfileCreatedByAdmin']);
        }
      }
